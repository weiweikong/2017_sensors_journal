...
\abstract{	
One of the greatest challenges for Unmanned Aircraft Vehicles(UAVs) applications is the safe landing. While UAV can land safely with the help of the global navigation satellite system(GNSS), the capability of autonomous landing is still limited in GNSS-impaired environments. In this paper, we propose a real-time vision-based localization framework that could track and locate the aircraft precisely by triangular geometry during the final approach. This long baseline system consists of two separate perception modules on the ground, which are equipped with optical sensors driving by a pan/tilt unit (PTU). To detect and track the landing target accurately at a long distance, we implement the Discriminative Scale Space Tracking(DSST) method in series with the Bounding Box Prediction algorithm. Both simulation and practical experiments prove that this novel visual system caters for a fixed-wing UAV landing assignment.}
...
\begin{figure}[!tb]
	...
	\subfigure[]
	{
		\includegraphics[height=4cm]{Figs/15_OPTAS1.pdf}
		\label{fig:15_OPTAS1}
	}
	\subfigure[]
	{
		\includegraphics[height=4cm]{Figs/30_UCARS2.pdf}
		\label{fig:30_UCARS2}
	}
...
\begin{figure}[!tb]
	...
	\subfigure[]
	{
		\includegraphics[height=5cm]{Figs/Fig08_Trinocular_system.pdf}
		\label{fig:Fig08_Trinocular_system}
	}
	\subfigure[]
	{
		\includegraphics[height=5cm]{Figs/Fig09_MultiCamera.pdf}
		\label{fig:Fig09_MultiCamera}
	}
